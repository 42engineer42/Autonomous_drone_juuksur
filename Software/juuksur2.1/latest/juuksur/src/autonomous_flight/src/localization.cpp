...
/*static float velMults[MAP_NODE_COUNT] = {
    1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 0.9f, 0.8f, 0.7f, 0.7f, 0.7f, 
    0.7f, 0.7f, 0.7f, 0.7f, 0.7f, 0.7f, 0.7f, 1.0f, 1.0f, 1.0f, 
    1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 
    1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 
    1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f
};*/
...
float Localization::compareParticleToMeasurement(LocParticle& p, float height, float pitch, float roll, V2 cpToGround, std::vector<ContourCenter>& cs, cv::Mat *debug) {
    Mat4 camToWorld;
    Quat rot, rollQ, pitchQ, yawQ;
    quat_angle_axis(&rollQ, -roll, ROLL_AXIS);
    quat_angle_axis(&pitchQ, -pitch, PITCH_AXIS);
...
V3 Localization::calculatePositionFromParticles() {
...
void Localization::update(float height, float roll, float pitch, float yaw, V2 hMovement, V2 cpToGround, float dt, std::vector<ContourCenter>& cs) {
	std::chrono::steady_clock::time_point start = std::chrono::steady_clock::now();
    float max = 0.0f;
    int maxi = 0;
    for(int i = 0; i < prevParticles.size(); i++) {
        prevParticles[i] = updateParticle(0.0f, hMovement, dt, prevParticles[i]);
        float w = compareParticleToMeasurement(prevParticles[i], height, pitch, roll, cpToGround, cs, NULL);
    ...
    V3 pos = calculatePositionFromParticles();
    this->lastEstimation = pos;
    drawParticles(cs, prevParticles[maxi]);

    this->prevParticles = particles;
}

// draw particle filter (for debugging)
void Localization::drawParticles(std::vector<ContourCenter>& cs, LocParticle bp) {
